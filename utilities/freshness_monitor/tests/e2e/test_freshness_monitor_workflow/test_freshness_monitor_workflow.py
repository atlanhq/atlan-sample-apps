...
class TestFreshnessMonitorWorkflow(unittest.TestCase, BaseTest):
    """E2E tests for Freshness Monitor workflow.

    Freshness Monitor is a workflow that:
    - Takes threshold_days as input
    - Monitors table freshness and tags stale tables
    ...
    def prepare_dir_paths(cls):
        ...
        Overrides base method to skip schema directory check since freshness_monitor
        ...
        # (not used for freshness_monitor since there's no data validation)
    ...
    def test_run_workflow(self):
        """
        Test running the freshness monitor workflow
        """
        self.run_workflow()
    ...
    @pytest.mark.skip(reason="Freshness Monitor workflow doesn't have auth endpoint")
    def test_auth(self):
    ...
    @pytest.mark.order(4)
    @pytest.mark.skip(
        reason="Freshness Monitor workflow doesn't have preflight check endpoint"
    )
    def test_preflight_check(self):
        """Skip - freshness_monitor doesn't have preflight check."""
        pass

    @pytest.mark.order(6)
    @pytest.mark.skip(
        reason="Freshness Monitor workflow doesn't have metadata for configuration update"
    )
    def test_configuration_update(self):
        """Skip - freshness_monitor doesn't have metadata for updates."""
        pass
    ...
    def test_auth_negative_invalid_credentials(self):
        """Skip - freshness_monitor doesn't have auth endpoint."""
        pass
    ...
    @pytest.mark.skip(
        reason="Freshness Monitor workflow doesn't require credentials/metadata"
    )

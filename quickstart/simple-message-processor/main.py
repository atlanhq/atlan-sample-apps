...
APPLICATION_NAME = "simple-message-processor"
...
async def process_messages_batch(messages: List[dict]):
    ...
    """
    logger.info("=" * 80)
    logger.info(f" HAHAHAHAHAHA Processing batch of {len(messages)} messages")
    logger.info("=" * 80)
    ...
    for idx, message in enumerate(messages, 1):
        ...
        # logger.info(
        #     f"Message {idx}/{len(messages)}: "
        #     f"Type={event_type}, Name={event_name}, Data={data}"
        # )

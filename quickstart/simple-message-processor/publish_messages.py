...
class DaprMessagePublisher:
    ...
    async def publish_message(self, message: dict) -> bool:
        """Publish a message to Kafka via Dapr pubsub.
        ...
        try:
            ...
            async with httpx.AsyncClient() as client:
                ...
                if response.status_code in [200, 204]:
                    ...
                    print(
                        f"[{datetime.now().strftime('%H:%M:%S')}] "
                        f"✗ Error #{self.error_count}: HTTP {response.status_code} - {response.text}"
                    )
                    return False
            ...
            print(
                f"[{datetime.now().strftime('%H:%M:%S')}] "
                f"✗ Error #{self.error_count}: Timeout connecting to Dapr"
            )
            return False
            ...
            print(
                f"[{datetime.now().strftime('%H:%M:%S')}] "
                f"✗ Error #{self.error_count}: Cannot connect to Dapr at {self.dapr_url}"
            )
            return False
...
async def main():
    ...
    parser.add_argument(
        ...
        type=int,
        default=3500,
        help="Dapr HTTP port (default: 3500)",
    )
    parser.add_argument(
        "--pubsub-name",
        default="eventstore",

...
class ActivitiesClass(ActivitiesInterface):
    ...
    def _transform_single_table_item(self, item: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            ...
            if item.get("Owner_Users")
            else [],
            "ownerGroups": item.get("Owner_Groups", "").split("\n")
            if item.get("Owner_Groups")
            else [],
    ...
    def _transform_table_data(
        ...
        for item in raw_data:
            ...
            if item.get("Type") != "Table":
                logger.warning(
                    f"Skipping non-Table asset: {item.get('Type', 'Unknown')}"
                )
                continue
    ...
    def _write_output_data(
        self, transformed_data: List[Dict[str, Any]], output_file: str
    ) -> None:
        """
        ...
        logger.info(
            f"Successfully wrote {len(transformed_data)} records to {output_file}"
        )

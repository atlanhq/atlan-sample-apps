...
class FreshnessMonitorActivities(ActivitiesInterface):  
    ...
    @activity.defn  
    async def fetch_tables_metadata(self, workflow_args: Dict[str, Any]) -> List[Dict[str, Any]]:  
                ...
                "query": {  
                    "bool": {  
                        "must": [  
                            ...
                            {"term": {"__state": "ACTIVE"}},
                            {"term": {"connectionQualifiedName": "default/snowflake/1703211402"}}
                        ]  
                    }  
                },  
                "from": 0,
        ...
        for asset in client.asset.search(search_request):  
            if isinstance(asset, Table):  
                ...
                count += 1
                logger.info(f"Processed table {count}: {table_info['name']} {table_info}")  
                
                # Break after processing the desired number of tables
                if count >= max_tables:
                    logger.info(f"Reached maximum limit of {max_tables} tables. Stopping.")
                    break
        ...
        for table in tables_data:  
            ...
            if update_time:  
                ...
                if isinstance(update_time, (int, float)):  
                    update_datetime = datetime.fromtimestamp(update_time / 1000)  # Assuming milliseconds  
                else:  
                    update_datetime = update_time  
                      
                if update_datetime < threshold_date:  
                    table["stale_since"] = threshold_date.isoformat()  # Convert to ISO string
    ...
    async def tag_stale_tables(self, args: Dict[str, Any]) -> Dict[str, Any]:  
        ...
        for table in stale_tables:  
            try:  
                logger.info(f"Adding stale data announcement for {table['qualified_name']}")
                ...
                response = client.asset.save(table_with_announcement)
                tagged_count += 1
                logger.info(f"✓ Stale data announcement added for {table['qualified_name']}")
                      
            except Exception as e:  
                logger.info(f"Failed to add announcement to table {table['qualified_name']}: {str(e)}")  
                failed_count += 1  
